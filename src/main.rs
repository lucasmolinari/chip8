...
const SCALE: u32 = 16;
const FPS: Duration = time::Duration::from_millis(16); // 16ms ~= 60Hz
const TICKS: u8 = 10;
...
pub fn main() -> Result<(), String> {
    ...
    if args.len() != 2 {
        eprintln!(
            "Expected one argument, got {}. Usage: cargo run -- <ROM path> ",
            args.len()
        );
        return Ok(());
    }
    ...
    'running: loop {
        ...
        if start.elapsed() < FPS {
            thread::sleep(FPS - start.elapsed());
        }
    }
...
fn draw(chip: &Chip8, canvas: &mut Canvas<Window>) -> Result<(), String> {
    ...
    for (i, pixel) in chip.display.iter().enumerate() {
        if *pixel == 1 {
            let x = (i % 64) as i32;
            let y = (i / 64) as i32;
